...
export class AvatarStepComponent {
  ...
  maxFileSizeMB = 5; // 5MB max file size
  ...
  onAvatarSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      // Check file size (convert maxFileSizeMB to bytes)
      if (file.size > this.maxFileSizeMB * 1024 * 1024) {
        this.avatarSizeError = true;
        this.avatarError = `Image too large! Maximum size is ${this.maxFileSizeMB}MB.`;
        return;
      }
  ...
  onCoverPhotoSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (file.size > this.maxFileSizeMB * 1024 * 1024) {
        ...
        this.coverPhotoError = `Image too large! Maximum size is ${this.maxFileSizeMB}MB.`;
        return;
      }
  ...
  uploadAvatar(): void {
    if (this.avatarFile) {
      ...
      this.onboardingService.uploadAvatar(undefined, this.avatarFile).subscribe({
        next: (response) => {
  ...
  uploadCoverPhoto(): void {
    if (this.coverPhotoFile) {
      ...
      this.onboardingService.uploadCoverPhoto(undefined, this.coverPhotoFile).subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          if (error.error && error.error.message) {
            ...
            this.coverPhotoError = 'Failed to upload image. Please try again.';
          }
        }
      });
    }
